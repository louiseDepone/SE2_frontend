...
export default function EmployeeOnLeave() {
  ...
  useEffect(() => {
    setEmployeeNavigate("Employee On Leave"); // Corrected function name
  }, []);

  const { employeeLoading, deleteemployee, Editemployee, singleEmployee } =
    useEmployeeContext();
  ...
  const getemployee = async () => {
    try {
      const del = await axios
        .get(`http://localhost:3000/employee/all`, {
          headers: {
            ...
            <div className="flex justify-center items-center  border-gray-200 border rounded-md h-7 pl-3 w-80   ">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setfiltered(
                    items.filter((a) => {
                      const d = a?.identification_number
                        .toString()
          ...
          <Table className="mt-5 bg-white rounded-md z-1">
            ...
            <TableBody>
              {currentItemsquery?.map((employee, index) => {
                return (
                  <TableRow
                    key={employee.identification_number + employee + index}
                  >
                    ...
                    <TableCell>
                      <span className="flex space-x-2 ">
                        {employee?.day_ids ? (
                          [...new Set(employee?.day_ids.split(","))].map(
                            (day, index) => {
                              return (
                                <span
                                  key={day + employee.identification_number}
                                  className={
                                    "text-sm flex justify-center nowrap items-center h-8 w-8 rounded-lg font-bold " +
                                    colorring[day]
                                  }
                                >
                                  {daysOfWeek[day]}
